<template>
	<div>
		<div class="grid">
			<div class="col-12">
				<div class="card">
					<div class="grid mb-3 col-12">
						<div class="col-12 text-left">
							<h1>{{ $store.state.tipo }}</h1>
						</div>
						<div class="formgroup-inline align-content-center justify-content-left col-9 col-md-12">
							<div class="field ml-2 mr-0 mb-0">
								...
								<div class="surface-300 border-round overflow-hidden w-10rem lg:w-8rem" style="height:10px">
									<div class="bg-purple-500 h-full"  v-bind:style="'width:' + ratiokcal + '%'"> </div>
								</div>
							...
							<div class="field ml-2 mr-1 mb-0">
								...
								<div class="surface-300 border-round overflow-hidden w-10rem lg:w-8rem" style="height:10px">
									<div class="bg-cyan-500 h-full"  v-bind:style="'width:' + ratioProteina + '%'"> </div>
								</div>
							...
							<div class="field ml-2 mr-0 mb-0">
								...
								<div class="surface-300 border-round overflow-hidden w-10rem lg:w-8rem" style="height:10px">
									<div class="bg-yellow-400 h-full"  v-bind:style="'width:' + ratioCarbohidrato + '%'"> </div>
								</div>
						...
						<div class="col-3 col-md-12 align-items-right">
					...
					<div class="grid col-12 card bg-surface-400 field">
						<div class="grid col-12 justify-content-around align-items-around">
							<div class="justify-content-left">
								<Button label="Favoritos" icon="pi pi-star" class="p-button-warning mr-2" size="sm" v-on:click="favoritos()"/>
								<Button label="Recientes" size="sm" class="mr-2" v-on:click="recientes()" />
								<Button label="Creados" size="sm" class="p-button-success mr-2" v-on:click="creados()"/>
							</div>
							<div>
								...
								<template #item="slotProps">
									<div class="col-12" style="text-align: center">
										<div class="carrousel-comidas">
											<div class="card">
												<div class="font-bold" style="margin: 4px">
													{{ slotProps.data.alimento.nombre }}
												</div>
												<div style="margin: 4px">
													Kcal: {{ slotProps.data.alimento.kcal_100g }}g. Cantidad: {{ slotProps.data.cantidad }}g
												</div>
												<div>
													<Button label="Quitar" style="margin: 4px" class="p-button-success" align="right" v-on:click="eliminarDelCarrusel(slotProps.data.alimento._id)"/>
												</div>
											</div>
										</div>
									</div>
			...
			>
				...
				<div class="col-12">
					...
					>
						...
						icon="pi pi-check"
						class="
							p-button-success p-button-icon-only p-button-rounded
						"
						></Button>
					</div>
					</div>
				...
				<div
					@click="detallesAlimento(slotProps.data)"
				...
				>
					...
					<div class="text-align-center">
						...
						<div class="col-8 text-left">
							...
							<div class="mb-3">
							Carbohidratos
							{{ slotProps.data.carbohidratos_100g }} g
							</div>
							<div class="mb-3">
							Proteínas {{ slotProps.data.proteinas_100g }} g
							</div>
		...
		<Dialog
			v-model:visible="alimentoDialog"
			header="Detalles del alimento"
			:modal="true"
			class="p-fluid"
			style="flex: 0 0 auto; width: 66.6667%"
			@close="this.imagenesAlergenos = []"
		>
			<div class="contenedor-imagen-detalles">
			<img
				:src="alimento.imagen_peq"
				:alt="alimento.nombre"
				class="mt-0 mx-auto mb-5 block shadow-2 imagen-comida-detalles"
			/>
			...
			<div class="grid justify-content-center mb-3">
				...
				>
					...
					<Button
						class="p-button"
						@click="anyadirConsumicion(alimento._id)"
					>
			...
			<div class="formgroup-inline justify-content-around">
				...
				<div class="formgroup-inline row flex justify-content-around">
					<div
					class="field"
					v-for="alergeno of obtenerAlergenos(alimento.alergenos)"
					:key="alergeno"
					>
					<span class="p-image p-component p-image-preview-container">
						<img
						:src="'images/alergenos/' + alergeno + '.svg'"
						width="100"
						:alt="alergeno"
						/>
					</span>
  ...
  methods: {
    enterClicked() {
      this.alimentoService
        .getBuscador(document.getElementById("BuscadorComidas").value)
        .then((data) => (this.dataviewValue = data));
    },
    ratios() {
      this.ratiokcal = Math.round(
        (this.dataviewValueComida["kcal_100g"] / this.kcal_recomendadas) * 100
      );
      console.log(this.kcal_recomendadas);
      console.log(this.ratiokcal);
      if (this.ratiokcal > 100) this.ratiokcal = 100;
      console.log(this.ratiokcal);
      this.ratioProteina = Math.round(
        (this.dataviewValueComida.proteinas_100g /
          this.proteinas_recomendadas) *
          100
      );
      if (this.ratioProteina > 100) this.ratioProteina = 100;
      console.log(this.ratioProteina);
      this.ratioCarbohidrato = Math.round(
        (this.dataviewValueComida.carbohidratos_100g /
          this.carbohidratos_recomendados) *
          100
      );
      if (this.ratioCarbohidrato > 100) this.ratioCarbohidrato = 100;
      console.log(this.ratioCarbohidrato);
      this.ratioGrasa = Math.round(
        (this.dataviewValueComida.grasa_100g / this.grasas_recomendadas) * 100
      );
    ...
    creados() {
      this.alimentoService
        .getCreados(this.$store.state.username)
        .then((data) => {
    ...
    alergenos() {
      this.alimentoService.getNoAlergeno(this.alergenosSel2).then((data) => {
        this.dataviewValue = data;
      });
    },
    comidaCarrusel() {
      this.alimentoService
        .getComida(
          this.$store.state.tipo,
          this.$store.state.fecha,
          this.$store.state.username
        )
        .then((data) => {
          this.dataviewValueComida = data;
          this.dataviewValueComida.kcal_100g =
            this.dataviewValueComida.kcal_100g.toFixed(2);
          this.dataviewValueComida.proteinas_100g =
            this.dataviewValueComida.proteinas_100g.toFixed(2);
          this.dataviewValueComida.carbohidratos_100g =
            this.dataviewValueComida.carbohidratos_100g.toFixed(2);
          this.dataviewValueComida.grasa_100g =
            this.dataviewValueComida.grasa_100g.toFixed(2);
          this.ratios();
          this.carrousel();
        });
    },
    ...
    userKcal() {
      this.alimentoService
        .getUserKcak(this.$store.state.username)
        .then((data) => {
          this.dataUserView = data;
          this.comidaCarrusel();
          this.kcal_recomendadas = (
            this.dataUserView.kcal_recomendadas / 3
          ).toFixed(2);
          this.carbohidratos_recomendados = (
            this.dataUserView.carbohidratos_recomendados / 3
          ).toFixed(2);
          this.proteinas_recomendadas = (
            this.dataUserView.proteinas_recomendadas / 3
          ).toFixed(2);
          this.grasas_recomendadas = (
            this.dataUserView.grasas_recomendadas / 3
          ).toFixed(2);
        });
    },
    ...
    favoritos() {
      this.alimentoService
        .getFavoritos(this.$store.state.username)
        .then((data) => (this.dataviewValue = data));
    },
    recientes() {
      this.alimentoService
        .getRecientes(this.$store.state.username)
        .then((data) => (this.dataviewValue = data));
    },
    ...
    anyadirConsumicion(alimentoId) {
      ...
      this.alimentoService
        .anyadirACarrusel(alimentoId, this.cantidad, this.dataviewValueComida)
        .then(() => {
          this.userKcal();
          this.alimentoService.nuevaConsumicion(
            alimentoId,
            this.$store.state.username
          );
